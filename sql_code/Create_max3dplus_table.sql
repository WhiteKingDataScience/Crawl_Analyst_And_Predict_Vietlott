...
CREATE TABLE max3d.prize_result_max3dplus (
    draw_ID INT,
    draw_date DATE,
    prize VARCHAR(50),
...
IF NOT EXISTS (
				...
				WHERE object_id = OBJECT_ID('vietlott.max3d.prize_result_max3dplus')
				AND name in ('idx_draw_date', 'idx_draw_id')
			)
BEGIN
CREATE INDEX idx_draw_date ON max3d.prize_result_max3dplus(draw_date)
CREATE INDEX idx_draw_id ON max3d.prize_result_max3dplus(draw_ID)
END
GO


CREATE OR ALTER TRIGGER max3d.Set_Winning_Condition_Max3DPlus ON max3d.prize_result_max3dplus
AFTER INSERT, UPDATE
AS
BEGIN
    UPDATE p
    SET p.winning_condition = CASE
        WHEN i.prize = 'Special Prize' THEN 'Match 2 sets of 3 numbers in any order'
    ...
    END
    FROM vietlott.max3d.prize_result_max3dplus p
    INNER JOIN inserted i ON p.draw_ID = i.draw_ID AND p.prize = i.prize;
END;


ALTER TABLE max3d.prize_result_max3dplus ENABLE TRIGGER  Set_Winning_Condition_Max3DPlus